...
static HI_S32 SAMPLE_Capture_VideoFrameProc(VI_PIPE ViPipe, VIDEO_FRAME_INFO_S* pstRawInfo)
{
    ...
    /* The interface HI_MPI_ISP_RunOnce can be set after the interafce HI_MPI_VI_SendPipeRaw,
       when the first step is HI_MPI_VI_SendPipeRaw, the second step is  HI_MPI_ISP_RunOnce,
       this order can save time,  the video stream is using statistics as same as HI_MPI_ISP_RUN. */
